...
export function isFolderScopePath(path) {
...
export function looksLikeFilePath(path) {
    if (isFolderScopePath(path))
        return false;
    const base = basename(path);
    if (!base)
        return false;
    if (KNOWN_ROOT_FILES.has(base))
        return true;
    if (base.startsWith('.') && base.length > 1)
        return true;
    return base.includes('.');
}
function rejectOne(path) {
    const p = normalizeScopePath(path);
    if (!p) {
...
export function validateInitialLockPaths(paths) {
    const normalized = [...new Set(paths.map(normalizeScopePath).filter(Boolean))].sort((a, b) => a.localeCompare(b));
...
export function validateExpandPaths(paths) {
    const normalized = [...new Set(paths.map(normalizeScopePath).filter(Boolean))].sort((a, b) => a.localeCompare(b));
    if (!normalized.length) {
        return {
            ok: false,
