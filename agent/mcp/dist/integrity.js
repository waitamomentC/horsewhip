 ...
 */
export function assertMcpIntegrityOrExit(entryPath) {
    ...
    if (actual !== expected) {
        console.error('[horsewhip-mcp] INTEGRITY CHECK FAILED — MCP binary hash mismatch.', 'Expected:', `${expected.slice(0, 16)}…`, 'Actual:', `${actual.slice(0, 16)}…`, 'Re-run Horsewhip: Configure Agent and reload the window.');
        process.exit(1);
    }
}
