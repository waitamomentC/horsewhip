...
一键安装（**推荐：VS Code 命令「配置 Agent」**；备选 CLI）：

```bash
# 推荐：装 Horsewhip 插件 → 命令面板 → Horsewhip: 配置 Agent（MCP + Skill）

# CLI（开发者）
cd /path/to/horsewhip && npm run build:extension
npm run setup:agent -- --project /path/to/your-git-app --from-extension ./extension
```

详见 [agent-setup.md](./agent-setup.md)。
...
```json
{
  "mcpServers": {
    "horsewhip": {
      "command": "node",
      "args": ["/…/extensions/horsewhip/media/mcp/dist/index.js"],
      "env": {
        "HORSEWHIP_WORKSPACE": "${CLAUDE_PROJECT_DIR}",
        "HORSEWHIP_MCP_VERSION": "2.1.0",
        "HORSEWHIP_MCP_HASH": "abc…"
      },
      "alwaysLoad": true
    }
  }
